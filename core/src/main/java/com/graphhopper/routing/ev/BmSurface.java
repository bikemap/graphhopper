/*
 *  Licensed to GraphHopper GmbH under one or more contributor
 *  license agreements. See the NOTICE file distributed with this work for
 *  additional information regarding copyright ownership.
 *
 *  GraphHopper GmbH licenses this file to you under the Apache License,
 *  Version 2.0 (the "License"); you may not use this file except in
 *  compliance with the License. You may obtain a copy of the License at
 *
 *       http://www.apache.org/licenses/LICENSE-2.0
 *
 *  Unless required by applicable law or agreed to in writing, software
 *  distributed under the License is distributed on an "AS IS" BASIS,
 *  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *  See the License for the specific language governing permissions and
 *  limitations under the License.
 */
package com.graphhopper.routing.ev;

import com.graphhopper.util.Helper;


import java.util.HashMap;
import java.util.Map;


public enum BmSurface {
    // Order is important to make ordinal roughly comparable
    MISSING,

    PAVED, ASPHALT, CONCRETE, PAVING_STONES, UNPAVED, GRAVEL, SHELLS, GROUND, SNOW, COBBLESTONE,
    METAL, WOOD, LOOSE_GRAVEL, PEBBLESTONE, WOODCHIPS, MUD, SAND, ROCK, METAL_GRID, OTHER;

    public static final String KEY = "bm_surface";

    private static final Map<String, BmSurface> SURFACE_MAP = new HashMap<>();

    static {
        for (BmSurface surface : values()) {
            if (surface == MISSING || surface == OTHER || surface == LOOSE_GRAVEL)
                continue;
            SURFACE_MAP.put(surface.toString(), surface);
        }

        // Asphalt additions
        SURFACE_MAP.put("chipseal", ASPHALT);

        // Paving Stones additions
        SURFACE_MAP.put("bricks", PAVING_STONES);

        // Gravel Additions
        SURFACE_MAP.put("compacted", GRAVEL);

        // Ground Additions
        SURFACE_MAP.put("earth", GROUND);
        SURFACE_MAP.put("dirt", GROUND);
        SURFACE_MAP.put("grass", GROUND);
        SURFACE_MAP.put("grass_paver", GROUND);
        SURFACE_MAP.put("salt", GROUND);

        // Cobblestone Additions
        SURFACE_MAP.put("sett", COBBLESTONE);
        SURFACE_MAP.put("unhewn_cobblestone", COBBLESTONE);

        // Loose Gravel
        SURFACE_MAP.put("fine_gravel", LOOSE_GRAVEL);

        // Snow Additions
         SURFACE_MAP.put("ice", SNOW);
    }

    public static EnumEncodedValue<BmSurface> create() {
        return new EnumEncodedValue<>(KEY, BmSurface.class);
    }

    @Override
    public String toString() {
        return Helper.toLowerCase(super.toString());
    }

    public static BmSurface find(String name) {
        if (Helper.isEmpty(name))
            return MISSING;

        int colonIndex = name.indexOf(":");
        if (colonIndex != -1) {
            name = name.substring(0, colonIndex);
        }

        return SURFACE_MAP.getOrDefault(name, OTHER);
    }

    public boolean isPleasant() {
        int surfaceOrdinal = this.ordinal();

        return MISSING.ordinal() < surfaceOrdinal && surfaceOrdinal <= SNOW.ordinal();
    }
}
