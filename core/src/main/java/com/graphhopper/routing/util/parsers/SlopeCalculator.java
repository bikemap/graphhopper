...
public class SlopeCalculator implements TagParser {
    ...
    public void createEncodedValues(EncodedValueLookup lookup, List<EncodedValue> registerNewEncodedValue) {
        if (maxSlopeEnc != null) {
            registerNewEncodedValue.add(maxSlopeEnc);
        }
    ...
    public IntsRef handleWayTags(IntsRef edgeFlags, ReaderWay way, boolean ferry, IntsRef relationFlags) {
        ...
        if (pointList != null) {
            ...
            if (averageSlopeEnc != null) {
                if (towerNodeSlope >= 0)
                    averageSlopeEnc.setDecimal(false, edgeFlags, Math.min(towerNodeSlope, averageSlopeEnc.getMaxDecimal()));
                else
                    averageSlopeEnc.setDecimal(true, edgeFlags, Math.min(Math.abs(towerNodeSlope), averageSlopeEnc.getMaxDecimal()));
            }
            ...
            if (maxSlopeEnc != null)
                maxSlopeEnc.setDecimal(false, edgeFlags, Math.min(maxSlope, maxSlopeEnc.getMaxDecimal()));
        }
        return edgeFlags;
    }
}